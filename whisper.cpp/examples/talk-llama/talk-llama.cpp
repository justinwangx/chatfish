...
void whisper_print_usage(int /*argc*/, char **argv,
  ...
  fprintf(
      stderr,
      "  -ngl N,   --n-gpu-layers N [%-7d] number of layers to store in VRAM\n",
      params.n_gpu_layers);
  fprintf(stderr,
  ...
  fprintf(stderr,
          ...
          "start dialog\n",
          "");
  fprintf(stderr, "  --session FNAME                   file to cache model "
                  "state in (may be large!) (default: none)\n");
...
int main(int argc, char **argv) {
  ...
  struct llama_model *model_llama =
      llama_load_model_from_file(params.model_llama.c_str(), lmparams);
  ...
  struct llama_context *ctx_llama =
      llama_new_context_with_model(model_llama, lcparams);
  ...
  // print some info about the processing
  {
    ...
    fprintf(stderr,
            "%s: processing, %d threads, lang = %s, task = %s, timestamps = %d "
            "...\n",
  ...
  const std::string prompt_whisper =
      ::replace(k_prompt_whisper, "{1}", params.bot_name);
  ...
  while (is_running) {
...

    {
      ...
      if (::vad_simple(pcmf32_cur, WHISPER_SAMPLE_RATE, 1250, params.vad_thold,
                       params.freq_thold, params.print_energy) ||
          force_speak) {
        // fprintf(stdout, "%s: Speech detected! Processing ...\n", __func__);

        // Read the recognized face name from the file
        std::ifstream name_file(
            "/home/bass/code/code_camera/recognized_face.txt");

        if (name_file.is_open()) {
          std::getline(name_file, recognized_name);
          name_file.close();
        } else {
          recognized_name
              .clear(); // Clear the name if the file couldn't be opened
        }
        ...
        if (!force_speak) {
          text_heard = ::trim(::transcribe(ctx_wsp, params, pcmf32_cur,
          ...
          if (embd.size() > 0) {
            if (n_past + (int)embd.size() > n_ctx) {
              ...
              embd.insert(embd.begin(),
                          embd_inp.begin() + embd_inp.size() - n_prev,
                          embd_inp.end());
        ...
        // Speak name
        if (!recognized_name.empty()) {
          text_to_speak = "Hi " + recognized_name + ", " + text_to_speak;
        }
